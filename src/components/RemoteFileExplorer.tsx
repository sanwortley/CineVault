...
import { Folder, ChevronRight, HardDrive, Search, X, Home, Monitor, Download, Files, Film } from 'lucide-react'
import { api } from '../api'
...
interface HomeFoldersResponse {
  home: string
  commonFolders: FsItem[]
}
...
const FolderIcon = ({ name }: FolderIconProps) => {
...
export default function RemoteFileExplorer({ onSelect, onClose }: RemoteFileExplorerProps) {
  const [path, setPath] = useState('/')
  const [items, setItems] = useState<FsItem[]>([])
  const [drives, setDrives] = useState<string[]>([])
  const [homeInfo, setHomeInfo] = useState<HomeFoldersResponse | null>(null)
    ...
    const init = async () => {
      try {
        const [drivesData, homeData] = await Promise.all([
          api.getDrives() as Promise<string[]>,
          api.getHomeFolders() as Promise<HomeFoldersResponse>
        ])
  ...
  const navigateTo = async (newPath: string) => {
    ...
    try {
      const content = await api.ls(newPath) as FsItem[]
