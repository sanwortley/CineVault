...
function MovieDetailsModal({ movie, onClose, onPlay, myList = [] as { id: number }[], toggleMyList }: MovieDetailsModalProps) {
  ...
  useEffect(() => {
    if (movie?.id && user) {
      api.getUserRating(user.id, movie.id).then(res => {
        const data = res as { rating?: number }
  ...
  useEffect(() => {
    if (!isLibraryMovie && isAdmin() && movie) {
      const fetchTorrents = async () => {
        ...
        try {
          const data = await api.findTorrents(movie.official_title || movie.title || '') as TorrentResult[]
          setTorrents(data)
        } catch (err) {
  ...
  const handleRequestMovie = async (m: DetailMovie) => {
  ...
  const handleDownload = async (torrent: TorrentResult) => {
    ...
    try {
      const result = await api.downloadMovie(
        ...
        { isPage: (torrent as unknown as Record<string, unknown>).isPage as boolean, isHash: (torrent as unknown as Record<string, unknown>).isHash as boolean }
      ) as { movieId?: number }
