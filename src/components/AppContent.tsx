...
interface LibraryPageProps {
  movies: Movie[] | MovieGroup[]
  isLoading: boolean
  onPlayMovie: (movie: DetailMovie) => void
  onInfoMovie: (movie: DetailMovie) => void
  search: string
  myList: Movie[]
...
const LibraryPage = lazy(() => import('../pages/LibraryPage')) as React.LazyExoticComponent<React.ComponentType<LibraryPageProps>>
const SettingsPage = lazy(() => import('../pages/SettingsPage')) as React.LazyExoticComponent<React.ComponentType<SettingsPageProps>>
const UploadPage = lazy(() => import('../pages/UploadPage')) as React.LazyExoticComponent<React.ComponentType<UploadPageProps>>
const ExplorePage = lazy(() => import('../pages/ExplorePage')) as React.LazyExoticComponent<React.ComponentType<ExplorePageProps>>
...
export default function AppContent() {
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      ...
      const mylistData = await getUserMylist() as { movie_id: number }[]
      ...
      const progressData = await getUserProgress() as { movie_id: number; watched_duration: number; updated_at: string; is_hidden: boolean }[]
  ...
  const groupedMovies = useMemo(() => groupMoviesByTitle(movies || []), [movies])
  ...
  const handleHideProgress = async (movieId: number) => {
  ...
  const [playerCrashError, setPlayerCrashError] = useState<string | null>(null)
    ...
    <div className="min-h-screen bg-black text-white selection:bg-netflix-red/30">
      ...
      {playingMovie && (
        <VideoPlayerErrorBoundary
          ...
          onError={(err: Error) => setPlayerCrashError(err.message || String(err))}
        >
          <VideoPlayer
            movie={playingMovie}
            userProgress={userProgress}
            onClose={(savedTime: unknown) => {
              setPlayingMovie(null)
              loadData(true)
            }}
            onOpenSettings={() => {
            ...
            onVersionChange={(newVersion: DetailMovie) => {
            ...
            <div className="flex items-center gap-1.5 md:gap-4 shrink-0">
              {isAdmin() && (
                <>
                  ...
                  <ActivityCenter onNavigate={(tab: string) => {
                    setActiveTab(tab)
                    window.scrollTo({ top: 0, behavior: 'smooth' })
                  }} />
      ...
      <main className="relative z-0 pb-20 lg:pb-0">
        ...
        {activeTab === 'explore' && (
          <div className="pt-14 md:pt-24">
            <ExplorePage onInfoMovie={(movie) => setDetailMovie(movie as unknown as DetailMovie)} onPlayMovie={(movie) => setPlayingMovie(movie as unknown as DetailMovie)} />
          </div>
        )}
        {activeTab === 'upload' && (
          <UploadPage
      ...
      {!detailMovie && !isChangePasswordOpen && !isAvatarPickerOpen && <Disclaimer />}
