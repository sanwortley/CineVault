...
export default function MovieNews() {
  ...
  useEffect(() => {
    const loadNews = async () => {
      try {
        const data = await api.fetchMovieNews() as NewsItem[]
        setNews(data)
      } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {news.map((item, index) => (
                  <div
                    ...
                    className="group relative h-[450px] rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl cursor-pointer block animate-fade-in-up hover:-translate-y-2.5 transition-transform duration-300"
