...
import { X, Star, Calendar, Clock, User, Users, Play, Download, Cloud, Plus, Check } from 'lucide-react';

function MovieDetailsModal({ movie, onClose, onPlay, onUpload, myList = [], toggleMyList }) {
            ...
            <div className="relative w-full max-w-5xl max-h-[90vh] glass-card overflow-hidden shadow-2xl rounded-3xl flex flex-col md:flex-row animate-in slide-in-from-bottom-8 duration-500">
                ...
                <div className="flex-1 overflow-y-auto bg-slate-950/40 p-8 md:p-12 text-white">
                    ...
                    <div className="flex flex-wrap gap-4 pt-4">
                        ...
                        {!drive_file_id && (
                            <button 
                                onClick={(e) => { onUpload(e); onClose(); }}
                                className="px-8 py-4 bg-slate-800/50 text-white border border-white/10 rounded-2xl font-bold flex items-center gap-3 hover:bg-slate-800 hover:border-netflix-red/50 transition-all duration-300"
                            >
                                <Cloud size={20} className="text-netflix-red" /> SUBIR A LA NUBE
                            </button>
                        )}
                    </div>
                </div>
