...
export default function EditMovieDialog({ movie, onClose, onUpdate }: EditMovieDialogProps) {
  ...
  const handleReidentify = async () => {
    ...
    setSuccess('')
    try {
      const res = await api.reIdentifyMovie(movie.id, formData.official_title, formData.detected_year) as { success: boolean; details: { official_title: string; release_date?: string; overview: string; poster_url: string; genres: string } }
      if (res.success) {
        setFormData({
      ...
      const error = err as Error
      setError('No se encontró match: ' + error.message)
    } finally {
