...
export default function DriveExplorer({ onSelect, onClose }: DriveExplorerProps) {
  const [path, setPath] = useState<PathItem[]>([{ id: 'root', name: 'Mi Unidad' }])
  ...
  const loadFolderContent = async (folderId: string) => {
    ...
    setError(null)
    try {
      const res = await api.listDriveFiles(folderId) as DriveListResponse
      if (res.files) {
