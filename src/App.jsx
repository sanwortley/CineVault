...
import { api } from './api';
import { Layout, Library, Settings as SettingsIcon, Film, Search, RefreshCw, Menu, X as CloseIcon, Check, Play, ChevronLeft, ChevronRight, Download, LogOut, RotateCw, Home, Bookmark, User, Lock, Key, ShieldCheck, ArrowRight, AlertCircle, Loader2, Upload } from 'lucide-react';
...
function AppContent() {
    ...
    useEffect(() => {
        if ('serviceWorker' in navigator) {
            // Watch for changes in the SW controller (new worker activated)
    ...
    const loadData = async (silent = false) => {
        ...
        try {
            ...
            // Load user mylist from Supabase
            const mylistData = await getUserMylist();
            // Get movie details for mylist
    ...
    // Load movies and user data on mount
    useEffect(() => {
        if (user) loadData();
    }, [user]);
    ...
    // --- MANEJO DEL BOTÓN ATRÁS (PARA NO SALIR AL LOGIN) ---
    useEffect(() => {
        const handlePopState = (e) => {
            if (playingMovie) {
                // Si el reproductor está abierto, lo cerramos
                setPlayingMovie(null);
                // Evitamos que siga navegando hacia atrás (ya consumimos el 'atrás' con el pushState previo)
            } else if (detailMovie) {
                setDetailMovie(null);
            }
        };
    ...
    // Cuando abrimos el reproductor o el detalle, pusheamos un estado al historial
    useEffect(() => {
    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            // Optional: window.location.reload(); after a short delay or just let the global event handle it
        ...
        <div className="min-h-screen bg-black text-white selection:bg-netflix-red/30">
            {playingMovie && (
                <VideoPlayer
                    ...
                    onClose={(savedTime) => {
                        setPlayingMovie(null);
                        // Refresh data (including progress) immediately after closing
                        loadData(true);
                    }}
                />
            )}
            ...
            <main className="relative z-0 -mt-4 pb-20 lg:pb-0">
                ...
                {activeTab === 'upload' && (
                    <UploadPage 
                        onUploadComplete={() => {
                            // Automatically refresh library metadata after upload
