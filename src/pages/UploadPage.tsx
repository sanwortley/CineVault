...
export default function UploadPage({ onUploadComplete }: UploadPageProps) {
    ...
    const [uploadStates, setUploadStates] = useState<Record<string, { status: string; progress: number }>>({});
    const [existingMovies, setExistingMovies] = useState<{ file_name: string; file_size: number }[]>([]);
    ...
    useEffect(() => {
        const newStates: Record<string, { status: string; progress: number }> = {};
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const activeExports: Promise<void>[] = [];
            ...
            const processQueue = async () => {
                while (queue.length > 0) {
                    ...
                    const uploadPromise = (async () => {
                        ...
                        try {
                            await api.uploadToLibrary([file], '', (p: number) => {
                                setUploadStates(prev => ({
                                    ...prev,
            ...
            setError(error.message || 'Error en la sincronización turbo');
        } finally {
                ...
                >
                    ...
                    <div className="border border-dashed border-white/10 rounded-[3rem] p-10 mt-8 relative opacity-60 hover:opacity-100 transition-opacity">
                        ...
                        <input 
                            ...
                            ref={folderInputRef} 
                            type="file" 
                            {...({ webkitdirectory: 'true', directory: 'true' } as React.HTMLAttributes<HTMLInputElement>)}
                            multiple
