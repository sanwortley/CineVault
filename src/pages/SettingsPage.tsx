...
function SettingsPage({ onClose, onTabChange }: SettingsPageProps) {
    ...
    const [stuckUploads, setStuckUploads] = useState<Record<string, unknown>[]>([]);
    const [isCheckingStuck, setIsCheckingStuck] = useState(false);

    const handleRemoveStuck = async (movieId: number, title: string) => {
    ...
    const fetchConfig = async () => {
        try {
            const foldersData = await api.getFolders() as { folder_path: string }[];
            setFolders(foldersData.map(f => f.folder_path));

            const key = await api.getTMDBKey() as string | undefined;
            if (key) { setApiKey(key); setIsSaved(true); }

            const oKey = await api.getOMDbKey() as string | undefined;
            ...
            if (isAdmin()) {
                const rdData = await api.getRDToken() as { token?: string };
                if (rdData.token) setRdToken(rdData.token);
                
                const osData = await api.getOSCredentials() as { username?: string; password?: string };
    ...
    const fetchStuckUploads = async () => {
        ...
        try {
            const stuck = await api.getStuckUploads() as Record<string, unknown>[];
    ...
    const handleRetryStuck = async (movieId: number) => {
        try {
                        ...
                        <div className="flex-1 space-y-3 md:space-y-4 mb-8 md:mb-12 bg-black/40 rounded-[1.5rem] md:rounded-[2.5rem] p-4 md:p-8 border border-white/[0.03] shadow-inner max-h-[300px] md:max-h-none overflow-y-auto">
                            {folders.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center py-10 md:py-20 opacity-30">
                                    <Folder size={64} strokeWidth={1} className="mb-4 md:mb-6" />
                                ...
                                folders.map((folder, index) => (
                                    <div key={index} className="flex justify-between items-center p-4 md:p-6 bg-white/[0.02] border border-white/5 rounded-xl md:rounded-2xl group hover:bg-white/[0.06] transition-all duration-500">
                                        <div className="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                                            <Folder size={20} className="text-slate-600 group-hover:text-netflix-red transition-colors shrink-0" />
                                    ...
                                    <div className="space-y-8">
                                        <div className="space-y-6">
                                            ...
                                            <div className="flex flex-col md:flex-row gap-4 md:gap-6">
                                                ...
                                                >
                                                    {isRDSaved ? <Check size={32} strokeWidth={4} /> : <span className="text-[10px] md:text-sm">Vincular Bóveda</span>}
                                                </button>
                                            </div>
                                        </div>
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {stuckUploads.map((movie) => {
                                            const stuckMovie = movie as Record<string, unknown>;
                                            const isRecoverable = !!stuckMovie.cloud_source_url;
                                            return (
                                                <div key={String(stuckMovie.id)} className="flex flex-col md:flex-row items-center justify-between p-6 bg-black/40 border border-white/5 rounded-[2rem] hover:bg-black/60 transition-all gap-6">
                                                    <div className="flex items-center gap-4 w-full">
                                                        <div className="w-12 h-18 bg-slate-800 rounded-lg overflow-hidden shrink-0">
                                                            {stuckMovie.poster_url ? <img src={stuckMovie.poster_url as string} alt="" className="w-full h-full object-cover" /> : null}
                                                        </div>
                                                        <div className="min-w-0 flex-1">
                                                            <h4 className="font-bold text-white uppercase tracking-tight truncate">{(stuckMovie.official_title || stuckMovie.detected_title) as string}</h4>
                                                            <div className="flex items-center gap-3 mt-1">
                                                                <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">ID: {stuckMovie.id as number}</p>
                                                                {!isRecoverable && (
                                                    ...
                                                    <div className="flex items-center gap-3 w-full md:w-auto">
                                                        {isRecoverable ? (
                                                            <button 
                                                                onClick={() => handleRetryStuck(stuckMovie.id as number)}
                                                            ...
                                                            <button 
                                                                onClick={() => handleRemoveStuck(stuckMovie.id as number, (stuckMovie.official_title || stuckMovie.detected_title) as string)}
