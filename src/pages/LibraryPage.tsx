...
const SelectField = ({ label, value, options, isOpen, onToggle, onSelect }: SelectFieldProps) => (
        ...
        {isOpen && (
            <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-zinc-900/95 backdrop-blur-2xl border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-[100] max-h-64 overflow-y-auto no-scrollbar py-2 animate-in fade-in slide-in-from-top-2 duration-200">
                {options.map(opt => (
                    <div
                        key={opt}
                        onClick={(e: React.MouseEvent) => { e.stopPropagation(); onSelect(opt); }}
...
function LibraryPage({
    ...
    const groupedMoviesRaw: MovieGroup[] = useMemo(() => groupMoviesByTitle(movies || []), [movies])

    const allGenres: string[] = ['All', ...new Set((groupedMoviesRaw || []).flatMap(m => (m && m.genres) ? m.genres.split(', ') : []))].sort()
    const allYears: string[] = ['All', ...new Set((groupedMoviesRaw || []).map(m => m?.detected_year).filter(Boolean) as string[])].sort((a: string, b: string) => Number(b) - Number(a))

    const moviesToFilter: (Movie | MovieGroup)[] = viewOnlyList
    ...
    const latestMovies = [...filteredMovies]
        ...
        .sort((a, b) => Number(b.detected_year || 0) - Number(a.detected_year || 0))
        .slice(0, 15)
        ...
        <div className="pb-40 min-h-screen bg-netflix-dark animate-fade-in" onClick={closeDropdowns}>
                ...
                {!viewOnlyList && (
                    <div className="flex flex-wrap items-center gap-3 md:gap-6 bg-white/5 p-3 md:p-4 rounded-2xl md:rounded-3xl backdrop-blur-3xl border border-white/10 shadow-2xl w-full lg:w-auto" onClick={e => e.stopPropagation()}>
                        ...
                        {(isFiltering || search !== '' || selectedSort !== 'A-Z') && (
                            <button
                                onClick={(e: React.MouseEvent) => { e.stopPropagation(); setSelectedGenre('All'); setSelectedYear('All'); setSelectedSort('A-Z') }}
            ...
            <div className="relative z-10 mt-16 pb-20">
                {isLoading && movies.length === 0 ? (
                    ...
                    <>
                        {!viewOnlyList && !isFiltering && search === '' && continueWatching.length > 0 && (
                            <div className="mb-20">
                        ...
                        <div className="px-3 md:px-16 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2 md:gap-8 animate-fade-in pb-32">
