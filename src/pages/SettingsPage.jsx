        ...
        <div className="p-12 pb-40 max-w-5xl mx-auto animate-fade-in relative z-10 no-drag">
            <header className="mb-20">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-[0.6em] text-cyan-500/90 glow-text">CineVault Experience</span>
                </div>
                <h1 className="text-7xl font-black tracking-tighter text-white mb-6 leading-none drop-shadow-2xl">Ajustes</h1>
                    ...
                    <div className="p-10 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                        <div className="flex items-center gap-6">
                            <div className="p-5 bg-cyan-500/10 rounded-3xl border border-cyan-500/20 shadow-inner">
                                <Folder className="text-cyan-500" size={32} strokeWidth={2.5} />
                            </div>
                            <div>
                        ...
                        <button
                            ...
                            disabled={isAdding}
                            className={`flex items-center gap-3 px-8 py-4 bg-cyan-500 text-black font-black rounded-[1.5rem] hover:scale-105 active:scale-95 transition-all duration-700 shadow-[0_20px_45px_-10px_rgba(6,182,212,0.4)] ${isAdding ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                        ...
                        {folders.length === 0 ? (
                            ...
                            folders.map((folder, index) => (
                                <div key={index} className="p-8 flex justify-between items-center hover:bg-white/[0.02] transition-all group">
                                    <div className="flex items-center gap-6">
                                        <div className="w-2.5 h-2.5 bg-cyan-500/20 rounded-full group-hover:bg-cyan-500 group-hover:shadow-[0_0_10px_rgba(6,182,212,0.8)] transition-all duration-500"></div>
                    ...
                    <div className="space-y-8">
                        <div className="p-10 bg-white/[0.01] border border-white/10 rounded-[2.5rem] shadow-inner">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                {isSaved && (
                                    <div className="flex items-center gap-2 text-cyan-500 animate-fade-in">
                                        <Check size={14} strokeWidth={4} />
                                        <span className="text-[10px] font-black uppercase tracking-widest">Configurado Correctamente</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-5">
                                <div className="relative flex-1 group">
                                    <ShieldCheck className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-cyan-500 transition-colors" size={20} />
                                    <input
                                        type="password"
                                        value={apiKey}
                                        onChange={(e) => {
                                        ...
                                        className="w-full bg-black/40 border border-white/10 rounded-[1.5rem] pl-16 pr-8 py-5 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500/30 transition-all duration-700 placeholder:text-slate-700"
                                    />
                                </div>
                                <button
                                    className={`px-10 py-5 glass-card rounded-[1.5rem] text-white font-black uppercase tracking-widest text-[10px] hover:text-cyan-500 transition-all duration-500 ${isSaved ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    onClick={() => {
                            ...
                            <div className="mt-8 flex items-center justify-between">
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    className="text-[10px] text-cyan-500 font-black uppercase tracking-widest hover:underline hover:glow-text transition-all flex items-center gap-2"
                                >
